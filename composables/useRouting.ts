...
//@ts-ignore
export function useRouting(map: L.Map, waypoints: IWayPointDto[], routeIndex: number = 0): Promise<{routing: L.Routing.Control, summary: any}> {
    return new Promise((resolve, reject) => {
        const {$L: L} = useNuxtApp()
        const colors = ['rgba(31,103,238,0.7)', '#0d26a8', '#19a90c', '#4974c9', '#000000']
        ...
        routing.on('routesfound', e =>  {
            ...
            L.polyline(coordinates, { color: colors[routeIndex], weight: 3 }).addTo(map)
